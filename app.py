...
def scan():
    ...
    if item:
        ...
        return redirect(_home_url(zone, shelf, focus='scan', msg=f"Added {item[1]} (+1) - now {new_qty} in stock", msgtype="ok"))
...
def new_item():
    ...
    return redirect(_home_url(zone, shelf, focus='scan', msg=f"Saved {name} (+1) - now 1 in stock", msgtype="ok"))
...
def resolve_barcode_page():
    ...
    if canonical:
        ...
        qty_msg = f" - now {int(item[3])} in stock" if item else ""
        return redirect(_home_url(zone, shelf, focus='scan', msg=f"Added {name} (+1){qty_msg}", msgtype="ok"))
        ...
        if action == "alias":
            ...
            if not canonical_barcode:
                ...
                try:
                    ...
                    qty_msg = f" - now {int(item[3])} in stock" if item else ""
                    return redirect(_home_url(zone, shelf, focus='scan', msg=f"Linked + Added {name} (+1){qty_msg}", msgtype="ok"))
...
def remove_one_route():
    ...
    return redirect(_home_url(zone, shelf, focus="remove", msg=f"Removed {item[1]} (-1) - now {new_qty} in stock", msgtype="danger"))
...
def inventory_remove():
    ...
    remove_one(barcode)
    new_qty = max(int(item[3]) - 1, 0)
    return redirect(f"/kitchen/inventory?msgtype=danger&msg=Removed%20{item[1].replace(' ', '%20')}%20(-1)%20-%20now%20{new_qty}%20in%20stock")
