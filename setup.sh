...
EOF
  # lat/lon will be resolved automatically by weather_client on first run
...
# Patch the service file on-the-fly to use the real user + real repo path
sed \
...
success "infopanel.service installed and started."
...
# Get the Pi's local IP for convenience
